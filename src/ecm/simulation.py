...
from .ecm_boundary_state import (
    ECMBoundaryState,
    SparseParticles,
    SparseBondTypes,
    SparseBonds,
...
from .parameters import EvolutionParameters
from .mpiwrap import CommWorld
...
class Simulation:
    ...
    def __init__(self, par: EvolutionParameters, ecm: MDState) -> None:
        ...
        box_z = par.box_size_z + box_offset * par.contour_length + safety_margin
        snapshot.configuration.box = [box_x, box_y, box_z, 0.0, 0.0, 0.0]
        print("Box = ", [box_x, box_y, box_z, 0.0, 0.0, 0.0])
        ...
        self._pos_offset = np.array(
            [-par.box_size_x / 2.0, -par.box_size_y / 2.0, -par.box_size_z / 2.0]
        )
    ...
    def get_state(self, skip_bonds=False) -> Optional[MDState]:
        ...
        with self._sim.state.cpu_local_snapshot as snapshot:
            ...
            l_par_pos = self._comm.gather(
                snapshot.particles.position[:, :] - self._pos_offset
            )
    ...
    def get_boundary_state(self) -> Optional[ECMBoundaryState]:
        ...
        with self._sim.state.cpu_local_snapshot as snapshot:
            ...
            local_par_pos = snapshot.particles.position[non_ghost_indices, :]
        ...
        if self.device.communicator.rank == 0:
            ...
            bond_types_r0 = np.array(
                [self._bond_force_cache[i][1] for i in bond_types_ids], dtype=np.float64
            )
            bond_types_k = np.array(
                [self._bond_force_cache[i][0] for i in bond_types_ids], dtype=np.float64
            )
            ...
            angle_cst_types_t0 = np.array(
                [self._angle_cst_force_cache[i][1] for i in angle_csts_typeids],
                dtype=np.float64,
            )
            angle_cst_types_k = np.array(
                [self._angle_cst_force_cache[i][0] for i in angle_csts_typeids],
                dtype=np.float64,
            )
            ...
            result = ECMBoundaryState(
                ...
                SparseAngleCsts(angle_csts_ids, angle_csts_groups, angle_csts_typeids),
            )
    ...
    def apply_interactions(self, interactions: CellECMInteractions) -> None:
        ...
        print(
            f"Asked to change { interactions.change_type_in_area.num_particles } particles"
        )
        print(
            f"Asked to move { interactions.move_adhesion_particles.new_pos.shape[0] } particles"
        )
        print(
            f"Asked to remove { interactions.remove_adhesion_particles.par_id.shape[0] } particles"
        )
        ...
        with self._sim.state.cpu_local_snapshot as snapshot:
            ...
            self._apply_adhesion_removal(
                interactions.remove_adhesion_particles, snapshot
            )

    def _apply_adhesion_removal(
        self,
        ...
        print(remove_adhesion_particles.par_id.dtype)
    ...
    def _apply_particle_type_changes(
        ...
        change_zone = {(x[0], x[1], x[2]) for x in change_type_in_area.change_area}

        potential_changes = list()
        for i in range(snapshot.particles.typeid.shape[0]):
            if snapshot.particles.typeid[i] == from_type_id:
                par_pos = snapshot.particles.position[i, :] - self._pos_offset
    ...
    def _apply_adhesion_particle_moves(
        self, move_adhesion_particles: MoveAdhesionParticles, snapshot: LocalSnapshot
    ) -> None:
    ...
    def _set_state_from_ecm(self, ecm: MDState) -> None:
        ...
        if self.device.communicator.rank == 0:
            ...
            snapshot.particles.position[:, :] = (
                ecm.particles.positions + self._pos_offset
            )
